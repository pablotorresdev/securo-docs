...
**Identificador Interno:** `CU23_VENTA_PRODUCTO`

---

## 1. DESCRIPCION

Permite registrar la venta de productos descontando stock del lote. El sistema descuenta cantidades por bulto, marca las trazas correspondientes como VENDIDO, y genera un movimiento de tipo BAJA/VENTA.

**Modos de operacion segun tipo de lote:**
- **Lote NO trazado:** El operador ingresa cantidades manualmente por bulto
- **Lote trazado:** El operador selecciona trazas especificas (cantidades se calculan automaticamente)

**Caracteristicas principales:**
- Solo lotes con dictamen LIBERADO pueden venderse
- Descuento multi-bulto (cantidades independientes por bulto)
- **UI condicional:** inputs de cantidad (lote NO trazado) o seleccion de trazas (lote trazado)
- Cantidades calculadas automaticamente desde trazas seleccionadas (lotes trazados)
- Validacion batch de trazas seleccionadas (1 query optimizada)
...
**Requisito:** El lote debe tener dictamen LIBERADO (previamente confirmado con CU22).
...
|  |  Fecha de egreso                                               |  |
|  |  [Si NO trazado] Cantidades por bulto (inputs numericos)      |  |
|  |  [Si trazado] Seleccion de trazas por bulto (checkboxes)     |  |
|  |               (cantidades calculadas automaticamente)         |  |
|  |  Motivo del cambio (min 20 chars)                             |  |
|  +---------------------------------------------------------------+  |
|  [Cancelar]                                   [Vista Previa] ->     |
+---------------------------------------------------------------------+
                              |
                              | POST /ventas/baja/venta-producto/confirm
                              v
+---------------------------------------------------------------------+
|  2. CONFIRMACION                                                     |
|     Template: ventas/baja/venta-producto-confirm.html               |
|  +---------------------------------------------------------------+  |
|  |  Vista previa de datos del lote                                |  |
|  |  Vista previa de cantidades por bulto a vender                 |  |
|  |  [Si trazado] Vista previa de trazas seleccionadas por bulto  |  |
...
| Archivo | Descripcion |
|---------|-------------|
| `templates/ventas/baja/venta-producto.html` | Formulario |
| `templates/ventas/baja/venta-producto-confirm.html` | Confirmacion |
| `templates/ventas/baja/venta-producto-ok.html` | Exito |

### Backend
| Archivo | Descripcion |
|---------|-------------|
| `controller/cu/BajaVentaProductoController.java` | Controlador |
| `service/cu/BajaVentaProductoService.java` | Servicio |
...
### 5.1 Obligatorios (todos los lotes)
...
### 5.2 Campos Condicionales (UI segun tipo de lote)
...
| Campo | HTML ID | Tipo | Validacion Browser | Validacion Server |
|-------|---------|------|-------------------|-------------------|
| Trazas | `trazaDTOs[n]` | checkbox | Al menos 1 seleccionada | Existencia, estado DISPONIBLE |

**Nota:** La UI cambia segun el tipo de lote:
- **NO trazado:** Se muestran inputs de cantidad por bulto
- **Trazado:** Se muestran checkboxes para seleccionar trazas; las cantidades por bulto se calculan automaticamente del numero de trazas seleccionadas por bulto
...
| `trazaDTOs[n].codigoLote` | Codigo del lote para cada traza seleccionada |
| `trazaDTOs[n].nroBulto` | Numero de bulto de la traza |
| `trazaDTOs[n].nroTraza` | Numero de la traza seleccionada |
...
| motivoDelCambio | `@NotNull`, `@Size(min=20)` | "El motivo del cambio es obligatorio (mínimo 20 caracteres)" |
...
| 7 | validarTrazasSeleccionadas | **[Solo lotes trazados con trazaDTOs]** Batch validation |

#### Validacion de Trazas Seleccionadas (validarTrazasSeleccionadas)

Solo se ejecuta cuando el lote es trazado Y hay trazaDTOs en el DTO:

1. **Batch query:** Usa `findAllByLoteCodigoLoteAndNroTrazaInAndActivoTrue()` (1 query, no N+1)
2. **Existencia:** Verifica que todas las trazas seleccionadas existan
3. **Estado DISPONIBLE:** Verifica que todas las trazas esten en estado DISPONIBLE

**Nota:** Las cantidades por bulto se calculan automaticamente desde las trazas seleccionadas, no se validan por separado.
...
3. **Carga dinamica condicional:** Al seleccionar lote:
   - **Si NO trazado:** Genera inputs de cantidad por cada bulto con stock > 0
   - **Si trazado:** Carga trazas disponibles con checkboxes por bulto
4. **Calculo automatico de cantidades:** Para lotes trazados, al submit se calculan las cantidades por bulto a partir de las trazas seleccionadas.
5. **Validacion:**
   - **NO trazado:** Al menos un bulto debe tener cantidad > 0
   - **Trazado:** Al menos una traza debe estar seleccionada
...
**POST /venta-producto/confirm** - Valida y muestra confirmacion

Ejecuta validaciones. Si ok, resuelve nombres del lote para mostrar en la vista de confirmacion y retorna el template de confirmacion. Si hay errores, retorna al formulario.

**POST /venta-producto** - Persiste venta

Recibe datos ya validados desde la pantalla de confirmacion. Ejecuta re-validacion, invoca `bajaVentaProducto()` y redirige a exito.
...
### 8.2.1 Metodo resolverNombresParaConfirmacion

Busca el lote completo por codigoLote y copia los datos necesarios para mostrar en la pantalla de confirmacion:
- nombreProducto, codigoProducto
- cantidadInicial, cantidadActual, unidadMedida
- bultosTotales, productoDestino
...
1. **Establecer contexto de auditoria:** "CU23_VENTA_PRODUCTO"
...
2. **Preparar mapa de trazas:** Si es seleccion manual (`trazaDTOs` no vacio), agrupa las trazas por `nroBulto`.

3. **Crear DetalleMovimiento por bulto:** Para cada bulto con cantidad > 0:
   - Crear DetalleMovimiento con bulto, cantidad, unidad
   - Si lote trazado:
     - **Seleccion MANUAL:** obtener trazas via `obtenerTrazasSeleccionadasPorBulto()`
     - **FIFO automatico:** obtener trazas via `getFirstAvailableTrazaList()`
   - Marcar trazas como VENDIDO
   - Guardar trazas

4. **Guardar movimiento**

### 9.4 Flujo segun Tipo de Lote

| Tipo de Lote | UI | Cantidades | Trazas |
|--------------|-----|-----------|--------|
| **NO trazado** | Inputs de cantidad | Ingresadas manualmente | N/A |
| **Trazado** | Checkboxes de trazas | Calculadas automaticamente | Seleccionadas por operador |

**Nota:** Para lotes trazados, las cantidades por bulto se derivan automaticamente del numero de trazas seleccionadas por bulto.

### 9.5 Metodo: obtenerTrazasSeleccionadasPorBulto

Filtra las trazas del bulto segun los `nroTraza` proporcionados:

```java
List<Traza> obtenerTrazasSeleccionadasPorBulto(Bulto bulto, List<Long> nroTrazas) {
    if (nroTrazas == null || nroTrazas.isEmpty()) {
        return new ArrayList<>();
    }
    Set<Long> nroTrazasSet = Set.copyOf(nroTrazas);
    return bulto.getTrazas().stream()
        .filter(Traza::getActivo)
        .filter(t -> nroTrazasSet.contains(t.getNroTraza()))
        .toList();
}
```
...
- **Contexto de caso de uso:** "CU23_VENTA_PRODUCTO"
- **Prefijo de CU:** `[CU23]`
...
- **Inicio:** `[CU23] Iniciando baja por venta - Usuario: {}, Lote: {}`
- **Completado:** `[CU23] Baja por venta completada - Lote: {}, Cantidad restante: {}, Estado: {}, MotivoDelCambio: {}`
...
| Prioridad | Mejora |
|-----------|--------|
| ~~BAJA~~ | ~~Agregar confirmacion antes de registrar venta~~ - **IMPLEMENTADO** |
| ~~BAJA~~ | ~~Mostrar preview de trazas que se van a marcar como VENDIDO antes de confirmar~~ - **IMPLEMENTADO** |
| ~~MEDIA~~ | ~~Cambiar de FIFO automatico a seleccion MANUAL de trazas para lotes trazados~~ - **IMPLEMENTADO** |
...
- **Archivo:** `venta-producto.html`
- **Problema:** Badge dice "CU Venta" en lugar de "CU23"
- **Impacto:** Solo visual/consistencia
- **Estado:** Corregido

### 16.2 Badge incorrecto en exito
- **Archivo:** `venta-producto-ok.html`
- **Problema:** Badge dice "CU Venta de Producto" en lugar de "CU23"
- **Impacto:** Solo visual/consistencia
- **Estado:** Corregido
...
### v2.1 (2025-12-27)
- **UX IMPROVEMENT:** UI condicional segun tipo de lote
  - Lotes NO trazados: inputs de cantidad por bulto
  - Lotes trazados: checkboxes de trazas (cantidades calculadas automaticamente)
  - Eliminada validacion redundante de coincidencia cantidad/trazas
  - Mejor UX: no confunde al usuario pidiendo cantidades Y trazas

### v2.0 (2025-12-27)
- **FEATURE:** Seleccion MANUAL de trazas para lotes trazados
  - UI con checkboxes por bulto para seleccionar trazas especificas
  - Validacion batch de trazas (1 query con IN clause, no N+1)
  - Vista previa de trazas seleccionadas en pantalla de confirmacion

### v1.1 (2025-12-26)
- Flujo de confirmacion intermedia implementado
