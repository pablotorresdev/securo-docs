...
**Identificador Interno:** `CU27_TRAZADO_LOTE`
...
**Referencia:** `PermisosCasoUsoEnum.CU27_TRAZADO` (linea 98-102)
...
|---------|-------------|--------|
| `templates/ventas/trazado/inicio-trazado.html` | Formulario principal | 201 |
| `templates/ventas/trazado/inicio-trazado-ok.html` | Pantalla de exito | 275 |

### Backend
| Archivo | Descripcion | Lineas |
|---------|-------------|--------|
| `controller/cu/ModifTrazadoLoteController.java` | Controlador | 108 |
| `service/cu/ModifTrazadoLoteService.java` | Servicio de negocio | 118 |
| `service/cu/AbstractCuService.java` | Clase base con validaciones comunes | 372 |

### Tests
| Archivo | Descripcion | Lineas |
|---------|-------------|--------|
| `service/cu/ModifTrazadoLoteServiceTest.java` | Tests del servicio | 247 |
| `controller/cu/ModifTrazadoLoteControllerTest.java` | Tests del controlador | 117 |
...
| POST | /ventas/trazado/inicio-trazado | Procesa trazado |
...
- **Flujo de 2 pasos:** Directo sin confirmacion intermedia.
...
1. **Establecer contexto de auditoria:** Registra "CU27_TRAZADO_LOTE" para Hibernate Envers.

2. **Obtener usuario actual:** Recupera el usuario autenticado via SecurityContextService.

3. **Log de inicio:** `[CU27] Iniciando trazado de lote - Usuario: {}, Lote: {}, TrazaInicial: {}`
...
8. **Crear Movimiento:** Llama a `persistirMovimientoTrazadoLote()`:
   ...
   - MotivoDelCambio: Formateado con prefijo `[CU27]`

9. **Log de completado:** `[CU27] Trazado de lote completado - Lote: {}, Trazas creadas: {}, MotivoDelCambio: {}`
...
| dictamenFinal | Dictamen actual del lote (sin cambio) |
| motivoDelCambio | `[CU27] ` + motivo del usuario |
| creadoPor | Usuario actual |
| activo | true |
...
- **Contexto de caso de uso:** "CU27_TRAZADO_LOTE" se guarda en cada revision via `CustomRevisionListener`.

- **Prefijo de CU:** El motivoDelCambio incluye `[CU27]` formateado con `MovimientoCommonUtils.formatMotivoDelCambioWithCU()`.
...
- **Inicio:** `[CU27] Iniciando trazado de lote - Usuario: {}, Lote: {}, TrazaInicial: {}`
- **Completado:** `[CU27] Trazado de lote completado - Lote: {}, Trazas creadas: {}, MotivoDelCambio: {}`

Todos los logs usan el prefijo `[CU27]` para facilitar la busqueda y correlacion.
...
| BAJA | Agregar preview de distribucion de trazas por bulto antes de confirmar |
