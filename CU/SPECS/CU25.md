...
**Identificador Interno:** `CU25_AJUSTE_STOCK`
...
|---------|-------------|--------|
| `templates/contingencias/ajuste-stock.html` | Formulario | 564 |
| `templates/contingencias/ajuste-stock-ok.html` | Exito | 322 |

### Backend
| Archivo | Descripcion | Lineas |
|---------|-------------|--------|
| `controller/cu/ContingenciasController.java` | Controlador (compartido con CU26) | 202 |
| `service/cu/BajaAjusteStockService.java` | Servicio | 190 |
| `service/cu/AbstractCuService.java` | Clase base | 371 |
...
| `dto/DTOUtils.java` | Conversion entidad -> DTO |
...
| Archivo | Descripcion | Lineas |
|---------|-------------|--------|
| `service/cu/BajaAjusteStockServiceTest.java` | Tests unitarios | 1344 |
...
| Motivo del cambio >= 20 chars | "El motivo del cambio es obligatorio (mínimo 20 caracteres)" |
...
| Carga de trazas | AJAX a `/trazas/disponibles/{codigoLote}/{nroBulto}` |
...
| `/trazas/disponibles/{codigoLote}/{nroBulto}` | GET | Obtiene trazas disponibles del bulto |
...
| POST | `/contingencias/ajuste-stock` | Procesa ajuste |
...
```
1. setCasoUsoContext("CU25_AJUSTE_STOCK")
   - Establece contexto para Hibernate Envers

2. getCurrentUser()
   - Obtiene usuario autenticado

3. log.info("[CU25] Iniciando ajuste...")
...
12. log.info("[CU25] Ajuste completado...")
    - Log de finalizacion

13. loteRepository.save(lote)
    - Persiste cambios (cascade)

14. return DTOUtils.fromLoteEntity(loteGuardado)
...
| motivoDelCambio | `[CU25] {texto del usuario}` |
| lote | Lote ajustado |
...
```java
setCasoUsoContext("CU25_AJUSTE_STOCK");
```
...
- Formato: `[CU25] {texto del usuario}`
...
- Roles definidos en `PermisosCasoUsoEnum.CU25_AJUSTE_INVENTARIO`
...
### Prefijo
Todos los logs usan el prefijo `[CU25]`
...
### Formato de Log
```
[CU25] Iniciando ajuste de stock - Usuario: {username}, Lote: {codigoLote},
       Bulto: {nroBulto}, Cantidad: {cantidad} {unidadMedida}

[CU25] Ajuste de stock completado - Lote: {codigoLote}, CantidadActual: {cantidad},
...
| Prioridad | Mejora |
|-----------|--------|
| Media | Soporte para multi-ajuste (varios bultos en una operacion) |
| Media | Confirmacion visual antes de submit (modal de resumen) |
| Baja | Historial de ajustes por lote (link directo) |
| Baja | Exportar reporte de ajustes a PDF/Excel |
| Baja | Notificacion a supervisores por ajustes significativos |
