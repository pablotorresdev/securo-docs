...
**Identificador Interno:** CU26_REVERSO_MOVIMIENTO

---

## 1. DESCRIPCION

El CU26 permite **revertir el ultimo movimiento registrado** de un lote. Es un caso de uso especial ya que:
...
[Se muestra ultimo movimiento]
      ...
      v
[POST /contingencias/reverso-movimiento]
      |
      +----------------+
      |                |
      v                v
...
[redirect 302]  [errorAutorizacion]
      |                |
      v                v
[GET /reverso-movimiento-ok]  [Formulario con error]
```
...
|---------|--------|-------------|
| `controller/cu/ContingenciasController.java` | 202 | Controller compartido CU25/CU26 |
| `service/cu/ModifReversoMovimientoService.java` | 174 | Servicio coordinador CU26 |
| `service/ReversoAuthorizationService.java` | 167 | Servicio de autorizacion |
| `service/cu/ReversoAltaService.java` | 297 | Reverso de ALTA (CU1, CU20, CU23, CU24) |
| `service/cu/ReversoBajaService.java` | 342 | Reverso de BAJA (CU3, CU4, CU7, CU22, CU25) |
| `service/cu/ReversoModificacionService.java` | 212 | Reverso de MODIF (CU2/8, CU5/6, CU11, CU21, CU27) |

### 4.2 Frontend

| Archivo | Lineas | Descripcion |
|---------|--------|-------------|
| `templates/contingencias/reverso-movimiento.html` | 260 | Formulario principal |
| `templates/contingencias/reverso-movimiento-ok.html` | 324 | Pantalla de exito |

### 4.3 Tests

| Archivo | Lineas | Descripcion |
|---------|--------|-------------|
| `service/cu/ModifReversoMovimientoServiceTest.java` | 717 | Tests del coordinador |

### 4.4 Enums/Config

| Archivo | Linea | Valor |
|---------|-------|-------|
| `enums/UseCaseTag.java` | 102 | `CU26("[CU26]")` |
| `enums/PermisosCasoUsoEnum.java` | 110-117 | Roles autorizados |
...
| CU24 (MODIF) | RETIRO_MERCADO | "El lote origen tiene un recall asociado, no se puede reversar el movimiento." |
...
### 8.2 Flujo POST (ContingenciasController.java:60-96)
...
public LoteDTO persistirReversoMovmiento(final MovimientoDTO dto) {
    1. setCasoUsoContext("CU26_REVERSO_MOVIMIENTO")
...
#### MODIFICACION (CU2/8, CU5/6, CU11, CU21, CU27)
```java
...
```java
setCasoUsoContext("CU26_REVERSO_MOVIMIENTO");
```
...
| Prefijo | Descripcion |
|---------|-------------|
| `[CU26]` | Operaciones del coordinador |
...
| `[REVERSO-CU21]` | Reverso confirmacion lote |
| `[REVERSO-CU22]` | Reverso venta |
| `[REVERSO-CU23]` | Reverso devolucion venta |
| `[REVERSO-CU24]` | Reverso retiro mercado |
| `[REVERSO-CU25]` | Reverso ajuste stock |
| `[REVERSO-CU27]` | Reverso trazado |
...
Todos los logs usan prefijo `[CU26]` o `[REVERSO-CU{n}]`

### 14.2 Puntos de Log

| Nivel | Punto | Mensaje |
|-------|-------|---------|
| INFO | Inicio | `[CU26] Iniciando reverso - Usuario: {}, Codigo: {}` |
| DEBUG | Validacion | `[CU26] Validando autorizacion - Mov: {}, Tipo: {}, Motivo: {}` |
| ERROR | No existe | `[CU26] Reverso fallido - Movimiento no existe: {}` |
| ERROR | Tipo null | `[CU26] Reverso fallido - Tipo de movimiento nulo` |
| ERROR | Multiples | `[CU26] Reverso fallido - Cantidad incorrecta: {}` |
| INFO | Exito | `[CU26] Reverso completado - Mov: {}, Lote: {}, Motivo: {}` |
...
| CU23 | ALTA | DEVOLUCION_VENTA | ReversoAltaService |
| CU24 | ALTA | RETIRO_MERCADO | ReversoAltaService |
...
| CU22 | BAJA | VENTA | ReversoBajaService |
| CU25 | BAJA | AJUSTE | ReversoBajaService |
...
| CU21 | MODIF | LOTE_LIBERADO | ReversoModificacionService |
| CU27 | MODIF | TRAZADO | ReversoModificacionService |
...
| CU24 (MODIF) | RETIRO_MERCADO | Recall asociado bloquea reverso |
